...
it("mostra o total de kcal do dia registrado no período atual", async () => {
  ...
  await createEntry(db, 1, { data: hoje(), meal_id: null, food_id: 1, qty_g: 100, label: null });
...
it("mostra água, atividades e balanço do período", async () => {
  ...
  await createEntry(db, 1, { data: hoje(), meal_id: null, food_id: 1, qty_g: 100, label: null }); // 128 kcal
...
it("mostra o painel de treino (sessões/volume/séries + grupo)", async () => {
  ...
  await addSet(db, 1, { session_id: s.id, exercise_id: 1, ordem: 1, reps: 10, peso_kg: 40 }); // volume 400
  ...
  expect(screen.getByText("peito")).toBeInTheDocument();     // breakdown por grupo
